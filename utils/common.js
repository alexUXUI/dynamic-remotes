...
export const importDelegatedModule = async (keyOrRuntimeRemoteItem) => {
  // @ts-ignore
...
export const createDelegatedModule = (delegate, params) => {
  const queries = [];
...
export const createRuntimeVariables = (remotes) => {
  if (!remotes) {
    return {};
  }
 ...
 */
export const getContainer = async (remoteContainer) => {
  if (!remoteContainer) {
